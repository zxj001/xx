...
public  interface ParquetTest {
  ...
  /**
   * Writes <code>data</code> to a Parquet file and reads it back as a SchemaRDD, which is then passed to <code>f</code>.
   * The Parquet file will be deleted after <code>f</code> returns.
   */
  public <T extends scala.Product> void withParquetRDD (scala.collection.Seq<T> data, scala.Function1<org.apache.spark.sql.SchemaRDD, scala.runtime.BoxedUnit> f, scala.reflect.ClassTag<T> evidence$3, scala.reflect.api.TypeTags.TypeTag<T> evidence$4) ;
  /**
  ...
  /**
   * Writes <code>data</code> to a Parquet file, reads it back as a SchemaRDD and registers it as a temporary
   * table named <code>tableName</code>, then call <code>f</code>. The temporary table together with the Parquet file will
   * be dropped/deleted after <code>f</code> returns.
   */
